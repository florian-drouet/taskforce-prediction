...
def plot_taskforce(y_true, y_future, population_type):
    ...
    trace_1 = go.Scatter(x=y_true.index,
                         y=y_true.values,
                         name="training")
    trace_2 = go.Scatter(x=y_future.index,
                         y=y_future.values,
                         name="predictions")
    ...
    # Add shape regions
    fig1.add_vrect(x0=y_future.index[0],
                   x1=y_future.index[-1],
                   fillcolor="LightGrey",
                   opacity=0.5,
                   layer="below",
    ...
    fig1.update_layout(plot_bgcolor='whitesmoke', title=f"{pop_translate[population_type]} taskforce prediction")
    fig1.update_yaxes(range=(0,max(y_true.max(), y_future.max())))
    
    return fig1

def plot_alert(alerts, y_future):
    trace_1 = go.Bar(x=alerts.index,
                     y=alerts.number_of_orange_alerts,
                     name="orange_alerts",
                     marker_color='gold')
    trace_2 = go.Bar(x=alerts.index,
    ...
    # Add shape regions
    fig2.add_vrect(x0=y_future.index[0],
                   x1=y_future.index[-1],
                   fillcolor="LightGrey",
                   opacity=0.5,
                   layer="below",
    ...
    fig2.update_layout(plot_bgcolor='whitesmoke', title="Evolution of red and orange alerts")
